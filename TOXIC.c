...
int main(int argc, char *argv[]) {
    ...
    if (local->tm_year + 1900 > EXPIRATION_YEAR ||
        ...
        printf("BINARY EXPIRED DM TO BUY :- @TOXICPLAYER002.\n");
        return EXIT_FAILURE;
    }

    printf("ATTACK STARTED BYE @TOXICPLAYER002\n");

    // Check for correct arguments
    if (argc != 5) { // Adjusted argument count
        printf("Usage: %s [ IP]  [ PORT ] [ TIME ]  [ PACKET_SENDER ] [ THREAD_COUNT ] | BYE @TOXICPLAYER002\n", argv[0]);
        return -1;
    }
    ...
    printf("Attack finished BYE @TOXICPLAYER002. All threads stopped.\n");
    return 0;
}
